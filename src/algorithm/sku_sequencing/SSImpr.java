...
public class SSImpr {
    ...
    public Solution cal(Solution solution) throws IOException, GRBException, CloneNotSupportedException {
        ...
        while(iter< 100){
            iter++;
            neigh = iter%3+1; // 定义邻域的序号
            neighSolution = findBestNeighbor(currentSolution.skuComeSeq, neigh, Math.min(this.combinationNumSKU, 100*this.instance.orderNum), currentSolution);
            ...
            this.tabuList.add(currentSolution.skuComeSeq.toString());
            if(this.tabuList.size()>30){
                this.tabuList.remove(0);
            }

            if (neighSolution.objVal < solution.objVal) {
                solution = neighSolution;
//                System.out.println("neigh="+neigh+",iter="+iter+",solutionObj="+solution.objVal +", skuComeSeq="+solution.skuComeSeq);
            }
        }
        // 输出最优值
        return solution;
    ...
    public Solution findBestNeighbor(List<Integer> skuComeSeq, int neigh, int neighborSize, Solution solution) throws IOException, GRBException, CloneNotSupportedException {
        // 声明变量
...
//                    continue;
//                }
//                neighSKUComeSeqFinal = checkIfFeasibleByGreedy(neighSKUComeSeq);
...
//            System.out.println(Arrays.deepToString(savedDistance));
            ...
            int idx1 = 0;
            int idx2 = 1;
            for(int idx = 0;idx<100*this.instance.orderNum;idx++){
                ...
                if (r1 != r2) {
                    neighSKUComeSeq= new ArrayList<>(skuComeSeq.subList(0, skuIdx1));
                    neighSKUComeSeq.add(skuComeSeq.get(skuIdx2));
                    neighSKUComeSeq.addAll(skuComeSeq.subList(skuIdx1 + 1, skuIdx2));
                    neighSKUComeSeq.add(skuComeSeq.get(skuIdx1));
                    neighSKUComeSeq.addAll(skuComeSeq.subList(skuIdx2 + 1, skuComeSeq.size()));
                    if(this.tabuList.contains(neighSKUComeSeq.toString())){
                        continue;
                    }
                    neighSKUComeSeqFinal = checkIfFeasibleByGreedy(neighSKUComeSeq);
    ...
    public List<Integer> checkIfFeasibleByGreedy(List<Integer> skuComeSeq){
        ...
        List<Integer> finalSKUComeSeq = new ArrayList<>();
        ...
        finalSKUComeSeq.addAll(skuComeSeq.subList(0, this.instance.toteCapByStation));
            ...
            while(pickingOrders.size()<this.instance.orderBinCap && remainingOrders.size()>0){
                ...
                if (removeSKUNum > 0) {
                    ...
                    while(activeSKU.size()<instance.toteCapByStation && nextSKUIndex != skuComeSeq.size()){
                        if(!activeSKU.contains(skuComeSeq.get(nextSKUIndex))){
                            finalSKUComeSeq.add(skuComeSeq.get(nextSKUIndex));
                        }
