...
public class VNS {
    ...
    public VNS(Instance instance, CalFitness cf){
        this.cf = cf;
    ...
    public Solution calVNS(Solution initialSolution) throws IOException, GRBException {
        ...
        while(iter <= AlgorithmParams.maxVNSIteration){
            ...
            while(neigh<=AlgorithmParams.maxNeighbourhoodNum){
                neighSolution = findBestNeighbor(shakedOrderSeq, neigh,  Math.min(instance.orderNum*2,AlgorithmParams.neighSearchSize));
            ...
            if (bestObj>shakedSolution.objVal) {
                bestObj = shakedSolution.objVal;
                bestSolution = shakedSolution;
                // 输出更新的最优解
                System.out.print("iter=" + iter  + ", obj = " + bestSolution.objVal + ", tote come num = " + bestSolution.skuComeSeq.size() + ", skuComeSeq = [");
                for (int sku : bestSolution.skuComeSeq) {
